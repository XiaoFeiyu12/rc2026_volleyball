...
IbvsControllerNode::IbvsControllerNode()
	: Node("ibvs_controller_node"), last_y_(0.0), smoothed_dist_(0.0), smoothed_y_(0.0), filter_inited_(false)
{
	// ── 参数 ──
	...
	ball_sub_ =
		this->create_subscription<Ball>("/detector/ball", rclcpp::SensorDataQoS(),
										std::bind(&IbvsControllerNode::ball_callback, this, std::placeholders::_1));

	// ── 发布到 /pid_camera（serial_driver 已订阅，复用 cmd=1） ──
	pid_camera_pub_ = this->create_publisher<PidCamera>("/pid_camera", rclcpp::SensorDataQoS());

	// ── 看门狗定时器 ──
	watchdog_timer_ = this->create_wall_timer(std::chrono::duration<double>(timeout_ / 2.0),
	...
	RCLCPP_INFO(this->get_logger(),
				...
				target_frame_.c_str(), Kp_lat_, Kp_long_, alpha_, timeout_);
}
...
void IbvsControllerNode::ball_callback(const Ball::SharedPtr msg)
{
	...
	{
		RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "TF 变换失败 (%s → %s): %s",
							 msg->header.frame_id.c_str(), target_frame_.c_str(), e.what());
		return;
	}

	RCLCPP_DEBUG(this->get_logger(), "球位置 (strike_point): x=%.3f y=%.3f z=%.3f", pt_base.point.x, pt_base.point.y,
				 pt_base.point.z);
	...
	if (pt_base.point.x < hit_offset_x_ && pt_base.point.x > 0.0 && std::abs(pt_base.point.y) < hit_offset_y_ &&
		std::abs(pt_base.point.z) < hit_offset_z_)
	{
		pid_msg.is_hit = 1;
	}
	else
...
void IbvsControllerNode::watchdog_callback()
{
	...
	if (dt > timeout_ && filter_inited_)
	{
		...
		RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "检测丢失 %.1fs → 零速", dt);
	}
}
