...
public:
	...
	virtual void set_state(const Eigen::VectorXd &x0) = 0;		   // 设置初始状态
	virtual Eigen::VectorXd get_state() = 0;						   // 获取当前状态
...
private:
	std::vector<Eigen::VectorXd> generate_sigma_points();	 // 生成Sigma点
	ProcessModel pm_;									 // 过程模型
	MeasureModel mm_;									 // 观测模型
