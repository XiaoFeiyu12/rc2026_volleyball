...
void Tracker::update(const BallSharedPtr& ball_msg, double dt)
{
    ...
    case STATE_IDLE:
    {
        ...
        ball_state << ball_msg->x, 0, ball_msg->y, 0,
                     std::max(0.0, static_cast<double>(ball_msg->z)), 0;
        KF_->setState(ball_state);
        ball_update_state_ = ball_state;
        track_state_ = STATE_DETECTING;
        detect_cnt_ = 1;
        lost_timer_ = 0.0;
        return;
    }
    ...
    case STATE_TEMP_LOST:
    {
        ...
        if (ball_update_state_(4) < 0.1) //0.1为排球半径
        {
            ...
            KF_->setState(ball_update_state_);
        }
        return;
    }
    }
