...
private:
    ...
    // 获取排球3D位置（深度图法）
    bool getBallPos_depth_img(Ball& volleyball, const DetectionBox& detect_box, const cv::Mat& depth_img);
    // 获取排球3D位置（几何估算法）
    void getBallPos_geometry(Ball& volleyball, const DetectionBox& detect_box);
