...
PredictNode::PredictNode() : Node("predict_node")
{
    ...
    target_ball_sub_ = this->create_subscription<volleyball_interfaces::msg::Ball>(
        ...
        std::bind(&PredictNode::TargetBallSubCallback, this, std::placeholders::_1));
    ...
    predict_trajectory_timer_ =
        this->create_wall_timer(std::chrono::milliseconds(static_cast<int>(1000.0f / predict_timer_freq)),
                                std::bind(&PredictNode::PredictTrajectoryTimerCallback, this));
 ...
 * @brief 球消息订阅回调：缓存球状态
 *****************************************************************/
void PredictNode::TargetBallSubCallback(const volleyball_interfaces::msg::Ball::SharedPtr msg)
{
    // 拷贝一份，避免修改原始消息
 ...
 *****************************************************************/
void PredictNode::PredictTrajectoryTimerCallback()
{
    if (!ball_cache_)
    {
        return;
    ...
    PublishTrajectoryMarker(trajectory);
 ...
 * @brief 发布预测轨迹可视化标记
 *****************************************************************/
void PredictNode::PublishTrajectoryMarker(const volleyball_interfaces::msg::BallTrajectory msg)
{
