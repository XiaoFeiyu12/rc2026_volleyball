...
private:
  ...
  void TargetBallSubCallback(const volleyball_interfaces::msg::Ball::SharedPtr msg);    // 球消息订阅回调
  void PredictTrajectoryTimerCallback();                                                 // 定时预测回调
  void PublishTrajectoryMarker(const volleyball_interfaces::msg::BallTrajectory msg);    // 发布轨迹可视化
};
