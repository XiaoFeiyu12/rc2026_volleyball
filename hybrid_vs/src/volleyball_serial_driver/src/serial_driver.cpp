  ...
  : rclcpp::Node(node_name), ctx{ IoContext(2) }, robotArray_ptr{ new robotArray }, planArray_ptr{ new planArray }
{
    ...
    pid_cam_sub_ = this->create_subscription<volleyball_interfaces::msg::PidCamera>(
        "/pid_camera", rclcpp::SensorDataQoS(), std::bind(&serial_driver_node::pid_cam_callback, this, std::placeholders::_1));
...
void serial_driver_node::serial_write_callback()
{
  ...
  {
    // pid_cam 模式：持续发送最新速度指令，保证下位机输入连续
    if (has_pid_cam_data_)
    {
        planArray_ptr->msg.header = 0xAA;
        planArray_ptr->msg.cmd = 1;
        planArray_ptr->msg.len = 16;
        planArray_ptr->msg.x = latest_pid_cam_.pixel_diff_x;
        planArray_ptr->msg.y = latest_pid_cam_.pixel_diff_y;
        planArray_ptr->msg.is_hit = latest_pid_cam_.is_hit;
        planArray_ptr->msg.landing_time = 0.0f;
        planArray_ptr->msg.my_xor = getContentXor(&planArray_ptr->array[1], sizeof(planArray_ptr->array) - 3);
        planArray_ptr->msg.tail = 0x55;

        serial_write(planArray_ptr->array, sizeof(planArray_ptr->array));

        // 仅在新数据到达时打印日志，避免 500Hz 刷屏
        if (has_new_pid_cam_)
        {
...
void serial_driver_node::robot_callback()
{
    if (isOpen && isRead)
    {
        try
        {
            ...
            RCLCPP_DEBUG(this->get_logger(), "x:%05.2f/y:%05.2f/self_pitch:%05.2f,mode:%d", robotArray_ptr->msg.x, robotArray_ptr->msg.y, robotArray_ptr->msg.self_pitch, robotArray_ptr->msg.mode);
...
void serial_driver_node::pid_cam_callback(volleyball_interfaces::msg::PidCamera::SharedPtr msg)
{
    latest_pid_cam_ = *msg;
    has_new_pid_cam_ = true;
