  ...
  : rclcpp::Node(node_name), ctx_{ IoContext(2) }, robot_array_ptr_{ new RobotArray }, plan_array_ptr_{ new PlanArray }
{
    ...
    ball_sub_ = this->create_subscription<volleyball_interfaces::msg::Ball>(
        "/detector/ball", rclcpp::SensorDataQoS(), std::bind(&SerialDriverNode::ball_callback, this, std::placeholders::_1));
...
void SerialDriverNode::serial_read_thread()
{
    while (rclcpp::ok())
    {
        ...
        try
        {
            ...
            // plan_array_ptr_->msg_.my_xor_ = get_content_xor(&plan_array_ptr_->array_[1], sizeof(plan_array_ptr_->array_) - 3);  // 更新校验值
...
void SerialDriverNode::robot_callback()
{
    if (is_open_ && is_read_)
    {
        try
        {
            ...
            RCLCPP_DEBUG(this->get_logger(), "x:%05.2f/y:%05.2f/self_pitch:%05.2f,mode:%d", robot_array_ptr_->msg_.x_, robot_array_ptr_->msg_.y_, robot_array_ptr_->msg_.self_pitch_, robot_array_ptr_->msg_.mode_);
