...
void TrackerNode::tryInitKF()
{
    ...
    auto mm = makePosition3D(sigma_pixel, sigma_depth_gain, sigma_depth_const, fx_, fy_, mm_ctx);
    auto pm = makeLinearDrag3D(Q_sigma_xy_min, Q_sigma_xy_max, Q_sigma_z_min, Q_sigma_z_max, k, m, g);
    ...
    ball_lost_selfcheck_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(static_cast<int>(selfcheck_time_thres * 1000)),
...
void TrackerNode::ball_lost_selfcheck_timer_callback()
{
    auto now = this->now();
    double dt = (now - last_time_).seconds();
    if (dt > lost_selfcheck_time_thres && tracker_->getState() != Tracker::STATE_IDLE)
    {
        tracker_->predictOnly(dt);
        last_time_ = now;
        if (tracker_->getState() != Tracker::STATE_IDLE)
        {
            publishTrackedBall(now);
        }
    }
 ...
 * @brief 发布跟踪结果及可视化标记
 *****************************************************************/
void TrackerNode::publishTrackedBall(const rclcpp::Time& stamp)
{
    auto pos = tracker_->getBallPosition();
    ...
    // 发布 /tracker/target
    {
        ...
        RCLCPP_DEBUG(this->get_logger(), "tracker_node发布滤波X=(%.1f,%.1f,%.1f),V=(%.1f,%1.f,%1.f)",
