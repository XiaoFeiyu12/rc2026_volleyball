...
EKF::EKF(const Eigen::MatrixXd& P0, ProcessModel pm, MeasureModel mm)
  : pm_(std::move(pm)), mm_(std::move(mm)),
    x_pred_(Eigen::VectorXd::Zero(6)), x_post_(Eigen::VectorXd::Zero(6)),
    P_post_(P0)
{
    P_pred_ = P_post_;
}
 ...
 *****************************************************************/
UKF::UKF(double alpha, double kappa, double beta, int n, int m, const Eigen::MatrixXd& P0,
         ProcessModel pm, MeasureModel mm)
  : pm_(std::move(pm)), mm_(std::move(mm)),
    alpha_(alpha), kappa_(kappa), beta_(beta), n_(n), m_(m), P_post_(P0)
{
    P_pred_ = P_post_;
 ...
 * @brief 设置UKF初始状态
 *****************************************************************/
void UKF::setState(const Eigen::VectorXd& x0)
{
    x_post_ = x0;
    x_pred_ = x_post_;
}
 ...
 *****************************************************************/
std::vector<Eigen::VectorXd> UKF::generateSigmaPoints()
{
...
{
    /*生成sigma点*/
    sigma_points_post_ = generateSigmaPoints();
    /*传播sigma点*/
...
Eigen::VectorXd UKF::update(const Eigen::VectorXd& z)
{
    ...
    /*计算卡尔曼增益*/
    Eigen::MatrixXd K =
        Pxz * S.ldlt().solve(Eigen::MatrixXd::Identity(S.rows(), S.cols()));
    /*更新*/
