...
DetectorNode::DetectorNode() : Node("detector_node"), frame_cnt_(0)
{
    ...
    color_info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
        "/camera/camera/color/camera_info", cam_info_qos,
    ...
    depth_info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
        "/camera/camera/depth/camera_info", cam_info_qos,
        [this](sensor_msgs::msg::CameraInfo::SharedPtr msg) {
            depth_info_ = *msg;
 ...
 *****************************************************************/
void DetectorNode::tryInitialize()
{
    ...
    cv::Mat color_cameraMatrix =
        (cv::Mat_<float>(3, 3) << static_cast<float>(color_info_.k[0]), static_cast<float>(color_info_.k[1]),
        static_cast<float>(color_info_.k[2]), static_cast<float>(color_info_.k[3]), static_cast<float>(color_info_.k[4]),
        static_cast<float>(color_info_.k[5]), static_cast<float>(color_info_.k[6]), static_cast<float>(color_info_.k[7]),
        static_cast<float>(color_info_.k[8]));
    ...
    for (size_t i = 0; i < 5; i++)
        color_camera_intrin.coeffs[i] = static_cast<float>(color_info_.d[i]);
    ...
    for (size_t i = 0; i < 5; i++)
        depth_camera_intrin.coeffs[i] = static_cast<float>(depth_info_.d[i]);
    ...
    Eigen::Vector3f depth_to_color_Translation(
        static_cast<float>(extrinsics_.translation[0]),
...
/*--------------------------------------------------------------------------------------------------*/
    ...
    sync_->registerCallback(
        std::bind(&DetectorNode::RGBDImgCallback, this, std::placeholders::_1, std::placeholders::_2));
    ...
    detector_ = std::make_shared<Detector>(
        this->get_logger(), param_model_path, cv::Size(640, 640), 
 ...
 * @brief RGBD图像回调
 *****************************************************************/
void DetectorNode::RGBDImgCallback(const sensor_msgs::msg::Image::ConstSharedPtr& rgb_msg,
                                   const sensor_msgs::msg::Image::ConstSharedPtr& depth_msg)
{
    try
    {
        ...
        if (!detector_->ball_list_.empty())
        {
            ...
            ball_msg.du = (detector_->detection_box_list_.front().cx - ppx_) * (rgb_img.cols / detector_->detection_box_list_.front().box.width );
        ...
        if (debug_ && (debug_frame_counter_++ % debug_frame_skip_ == 0))
        {
            ...
            detector_->drawDetectResult(rgb_show, detector_->detection_box_list_);
            ...
            if (!detector_->detection_box_list_.empty())
            {
                ...
                cv::circle(rgb_show, cv::Point(static_cast<int>(best_box.cx), static_cast<int>(best_box.cy)),
 ...
 *****************************************************************/
void DetectorNode::createDebugPublisher()
{
