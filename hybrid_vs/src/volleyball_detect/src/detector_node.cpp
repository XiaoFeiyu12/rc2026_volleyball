                                   ...
                                   const sensor_msgs::msg::Image::ConstSharedPtr& depth_msg)
{
    try
    {
        ...
        if (!detector_->ball_list_.empty())
        {
            ...
            ball_msg.du = (detector_->detection_box_list_.front().cx_ - ppx_) * (rgb_img.cols / detector_->detection_box_list_.front().box_.width );
        ...
        if (debug_ && (debug_frame_counter_++ % debug_frame_skip_ == 0))
        {
            ...
            if (!detector_->detection_box_list_.empty())
            {
                ...
                cv::circle(rgb_show, cv::Point(static_cast<int>(best_box.cx_), static_cast<int>(best_box.cy_)),
                        5, cv::Scalar(0, 0, 255), 2);
                cv::line(rgb_show, cv::Point(ppx_, ppy_),
                        cv::Point(static_cast<int>(best_box.cx_), static_cast<int>(best_box.cy_)),
                        cv::Scalar(255, 0, 0), 2);
            }
